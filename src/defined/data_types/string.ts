...
export function calcUtf16Length(str: string) {
  let utf8Len = 0;
  let code: number;
  let i = 0;
  let max = str.length;
...
function writeByUtf16Into(str: string, buf: Uint8Array, offset = 0) {
  let code;
  let max = str.length;
  let i = 0;
  while (i < max) {
...
export const encodeUtf8Into: (str: string, buf: Uint8Array, offset: number) => number = (function () {
  ...
  if (TextEncoder) {
    ...
    return (str: string, buf: Uint8Array, offset) => {
      ...
      return writeByUtf16Into(str, buf, offset);
    };
  } else return writeByUtf16Into;
})();
